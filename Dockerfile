...
# Instale o ChromeDriver e outras dependências necessárias
RUN apt-get update \
    && apt-get install -y wget unzip \
    && wget -q -O chromedriver.zip https://chromedriver.storage.googleapis.com/114.0.5735.90/chromedriver_linux64.zip \
    && unzip chromedriver.zip -d /usr/local/bin/ \
    && rm chromedriver.zip \
    && apt-get install -y libglib2.0-0 libnss3 libx11-xcb1 libxkbcommon0 libxcomposite1 libxdamage1 libxrandr2 libatspi2.0-0 \
    && apt-get clean \
